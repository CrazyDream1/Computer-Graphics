...
{
	UINT dxgi_fzctory_flags = 0;
#ifdef _DEBUG
	ComPtr<ID3D12Debug> debug_controller;
	if (SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&debug_controller))))
	{
		debug_controller->EnableDebugLayer();
		dxgi_fzctory_flags |= DXGI_CREATE_FACTORY_DEBUG;
	}
#endif // _DEBUG
...
void cg::renderer::dx12_renderer::load_assets()
{
	...
	root_parameter[0].InitAsDescriptorTable(1, &ranges[0], D3D12_SHADER_VISIBILITY_VERTEX);
	...
	rs_description.Init_1_1(_countof(root_parameter), root_parameter, 0, nullptr,  rs_flags);

	ComPtr<ID3DBlob> signature;
	ComPtr<ID3DBlob> error;

	HRESULT result = D3DX12SerializeVersionedRootSignature(
		&rs_description, rs_feature_data.HighestVersion, &signature, &error);
	if (FAILED(result))
	{
...
void cg::renderer::dx12_renderer::populate_command_list()
{
	...
	THROW_IF_FAILED(
		command_list->Reset(command_allocators[frame_index].Get(), pipeline_state.Get()));
